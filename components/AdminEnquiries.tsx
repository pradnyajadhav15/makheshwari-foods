  ...
  return (
    <section>
      <div className="flex items-end justify-between mb-7">
        ...
        <span className="text-ink/70 text-sm">{rows.filter((e) => e.status === "new").length} new</span>
      </div>

      {rows.length === 0 && <p className="text-ink/70 font-light">No enquiries yet.</p>}
          ...
          <div key={e.id} className={`bg-white rounded-[1.25rem] border p-7 ${e.status === "new" ? "border-gold/60" : "border-ink/10"}`}>
            <div className="flex flex-wrap justify-between gap-4 mb-5">
              <div>
                ...
                <p className="text-ink/70 text-sm mt-1">{e.company || "No business name"}{e.city ? ` · ${e.city}` : ""}</p>
                <p className="text-ink/70 text-xs mt-1">{new Date(e.created_at).toLocaleString("en-IN")}</p>
              </div>
            ...
            <div className="grid sm:grid-cols-3 gap-5 text-sm font-light border-t border-ink/10 pt-5">
              <div><p className="text-ink/70 text-[10px] tracking-tracksm uppercase mb-1.5">Type</p><p className="text-ink/70">{e.business_type || "-"}</p></div>
              <div><p className="text-ink/70 text-[10px] tracking-tracksm uppercase mb-1.5">Product</p><p className="text-ink/70">{e.product || "-"}</p></div>
              <div><p className="text-ink/70 text-[10px] tracking-tracksm uppercase mb-1.5">Quantity</p><p className="text-ink/70">{e.quantity || "-"}</p></div>
            </div>
            {e.notes && <p className="text-ink/70 text-sm font-light mt-4">{e.notes}</p>}
            <div className="flex gap-3 mt-6 pt-5 border-t border-ink/10">
              ...
              {confirm === e.id ? (
                <span className="flex gap-2 ml-auto">
                  ...
                  <button type="button" onClick={() => setConfirm(null)} className="border border-ink/20 rounded-full px-5 py-2 text-[10px] tracking-tracksm uppercase text-ink/70">Keep</button>
                </span>
              ) : (
