...
export default function PasswordInput({ value, onChange, placeholder = "Password", onEnter }: { value: string; onChange: (v: string) => void; placeholder?: string; onEnter?: () => void }) {
  ...
  return (
    <div className="relative">
      ...
      />
      <button type="button" onClick={() => setShow(!show)} aria-label={show ? "Hide password" : "Show password"} className="absolute right-1 top-1/2 -translate-y-1/2 w-11 h-11 flex items-center justify-center text-ink/40 hover:text-ink transition">
        {show ? (
