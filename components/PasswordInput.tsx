  ...
  return (
    <div className="relative">
      <input
        ...
        className="w-full bg-cream/60 border border-ink/15 rounded-xl px-5 py-4 pr-14 text-sm text-ink placeholder:text-ink/35 focus:outline-none focus:border-gold transition"
      />
      <button type="button" onClick={() => setShow(!show)} aria-label={show ? "Hide password" : "Show password"} className="absolute right-4 top-1/2 -translate-y-1/2 text-ink/40 hover:text-ink transition">
        {show ? (
