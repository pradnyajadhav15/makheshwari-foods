...
export default function AdminReviews() {
  ...
  return (
    <section>
      ...
      {rows.length === 0 && <p className="text-ink/70 font-light">No reviews yet.</p>}
          ...
          <div key={r.id} className={`bg-white rounded-[1.25rem] border p-7 ${r.approved ? "border-ink/10" : "border-gold/60"}`}>
            <div className="flex flex-wrap justify-between gap-4 mb-4">
              <div>
                ...
                <p className="text-ink/70 text-xs mt-1">{r.product_slug}{r.city ? ` \u00B7 ${r.city}` : ""} \u00B7 {new Date(r.created_at).toLocaleDateString("en-IN")}</p>
              </div>
            ...
            <p className="text-ink/75 text-sm font-light leading-relaxed mb-5">{r.body}</p>
